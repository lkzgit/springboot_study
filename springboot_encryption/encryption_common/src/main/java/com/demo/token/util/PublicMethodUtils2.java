 ...
 * @description  操作获取token工具类 原始不动修改
 */
public class PublicMethodUtils2 {

    private static PublicMethodUtils2 instance = null;


    /**
     * 通过构造方法进行初始化
     */
    private PublicMethodUtils2() {
    }

    public static PublicMethodUtils2 getInstance() {
        if (null == instance) {
            instance = new PublicMethodUtils2();
        }
        return instance;
    }
    ...
    url:对方请求服务的地址
     */
    public  static String getPublicKey(Map<String,byte[]> puPrKeyPair,String url){
        // 获取自己的公钥
        byte[] publicKeys = puPrKeyPair.get("publicKey");
        String base64String = Base64.encodeBase64String(publicKeys);
        //对方的公钥
        String oppoPublicKey = HttpRequest.get(url).header("publicKey", base64String).header("sysTem","sysTem").execute().body();
    ...
    public static String getToken(Map<String,byte[]>puPrKeyPair,String url,Map<String,Object> serveInfo,String aesKey ){
        ...
        try {
            ...
            //请求对方服务获取token
            String returnMessage = HttpRequest.post(url).header("message", askMessage).header("sysTem","sysTem").execute().body();
      ...
      //      boolean b = EncryptUtil.getInstance().verifySignature(split[0], split[1]);
            ...
            if(b){
                ...
                //byte[] tokenEncrypt = DHUtil.decrypt(split[0].getBytes(), secretKey);
